"""Noms de pistes, tags XML et comparaison à l'état vise (aucun outil externe requis)."""
...
class TestNomsDePistes(unittest.TestCase):
    ...
    def test_audio_sans_debit_connu(self):
        self.assertEqual(mkv.audio_track_name(piste("audio", codec="AAC", audio_channels=2)), "AAC 2.0")
    ...
    def test_selecteurs_numerotes_par_type(self):
        info = {"tracks": [piste("video"), piste("audio"), piste("subtitles"), piste("audio"), piste("subtitles")]}
...
class TestDrapeauxDeduitsDuNom(unittest.TestCase):
    ...
    def deduits(self, subs):
        """{sélecteur: drapeaux à poser} tels que le script les calcule."""
        return {sel: aj for sel, _, aj in mkv.subtitle_targets(subs, self.opts) if aj}

    def test_nom_force_pose_le_drapeau(self):
        # Régression : la piste devenait "Full" à côté de la piste complète, elle aussi "Full" - deux noms identiques, l'information perdue.
        subs = self.subs(piste("subtitles", language="fre", track_name="Français forcé"), piste("subtitles", language="fre", track_name="Français complet"))
    ...
    def test_nom_sdh_pose_le_drapeau(self):
        # Même défaut, symétrique : "English SDH" sans le drapeau malentendant devenait "Full" à côté de "English full", lui aussi "Full".
        subs = self.subs(piste("subtitles", language="eng", track_name="English full"), piste("subtitles", language="eng", track_name="English SDH"))
    ...
    def test_malentendant_ecrit_en_toutes_lettres(self):
        subs = self.subs(piste("subtitles", language="fre", track_name="Français malentendants"))
        self.assertEqual(self.deduits(subs), {"s1": {SDH}})

    def test_les_deux_drapeaux_a_la_fois(self):
        subs = self.subs(piste("subtitles", language="fre", track_name="Français forcé SDH"))
    ...
    def test_rien_dans_une_langue_qui_declare_deja_le_drapeau(self):
        # là où il existe, la situation est déclarée : un nom ne la contredit pas, et un second forced ferait choisir le lecteur au hasard.
        subs = self.subs(piste("subtitles", language="fre", forced_track=True, track_name="forced colored"), piste("subtitles", language="fre", track_name="Français forcé"))
        self.assertEqual(self.deduits(subs), {})

    def test_mais_une_autre_langue_reste_libre(self):
        # Une piste forcée anglaise ne dit rien de la française.
        subs = self.subs(piste("subtitles", language="eng", forced_track=True, track_name="English forced"), piste("subtitles", language="fre", track_name="Français forcé"))
        self.assertEqual(self.deduits(subs), {"s2": {FORCE}})

    def test_une_seule_piste_par_langue(self):
        subs = self.subs(piste("subtitles", language="fre", track_name="Français forcé"), piste("subtitles", language="fre", track_name="forcé colored"))
        self.assertEqual(self.deduits(subs), {"s1": {FORCE}})

    def test_nom_muet_ne_deduit_rien(self):
        subs = self.subs(piste("subtitles", language="fre", track_name="Français complet"), piste("subtitles", language="eng", track_name="English full"))
        self.assertEqual(self.deduits(subs), {})
    ...
    def test_verify_reclame_le_drapeau(self):
        # Sans cet écart, --skip-done sauterait le fichier à corriger.
        info = {"container": {"properties": {"title": "X"}}, "tracks": [piste("subtitles", language="eng", track_name="English SDH")]}
        diffs = [lbl for lbl, ok, _ in mkv.verify(info, mkv.Target(title="X"), self.opts) if not ok]
    ...
    def test_ecriture_pose_les_drapeaux_et_le_nom(self):
        ...
        info = {"tracks": [piste("subtitles", language="eng", track_name="English full SDH")]}
        with mock.patch.object(mkv.subprocess, "run", run):
            code, _ = mkv.write("film.mkv", info, mkv.Target(title="X", tags_xml="<Tags/>"), self.opts, None)
    ...
    def test_second_passage_ne_change_plus_rien(self):
        # Idempotence : le drapeau posé, le nom en découle de lui-même.
        info = {"container": {"properties": {"title": "X"}}, "tracks": [piste("subtitles", language="fre", forced_track=True, track_name="Forced")]}
...
class TestPistesAmbigues(unittest.TestCase):
    ...
    def test_forced_non_transposable_et_noms_en_double(self):
        # L état exact d Aquaman : une piste forcée déclarée, une deuxième qui ne l est que par son nom, et la complète. Les deux dernières deviendraient "Full" toutes les deux.
        raisons = self.conflits(piste("subtitles", language="fre", forced_track=True, track_name="forced colored"), piste("subtitles", language="fre", track_name="Français forcé"), piste("subtitles", language="fre", track_name="Français complet"))
    ...
    def test_deux_sous_titres_indistinguables(self):
        # L état exact de Catwoman : deux pistes anglaises que rien ne sépare.
        raisons = self.conflits(piste("subtitles", language="eng", track_name="English"), piste("subtitles", language="eng", track_name="English"))
    ...
    def test_fichier_sain(self):
        # Le cas courant : un force sans drapeau (qu on posera) et une complète.
        self.assertEqual(self.conflits(piste("subtitles", language="fre", track_name="Français forcé"), piste("subtitles", language="fre", track_name="Français complet")), [])

    def test_no_flags_rend_le_nom_force_intransposable(self):
        sans = mkv.Options(cover=False, date=False, stats=False, flags=False)
        raisons = self.conflits(piste("subtitles", language="fre", track_name="Français forcé"), opts=sans)
        self.assertIn("--no-flags", raisons[0])

    def test_sans_renommage_rien_a_signaler(self):
        muet = mkv.Options(cover=False, date=False, stats=False, audio_names=False, sub_names=False)
        self.assertEqual(self.conflits(piste("subtitles", language="eng", track_name="English"), piste("subtitles", language="eng", track_name="English"), opts=muet), [])
...
class TestIdentifiantDansLeFichier(unittest.TestCase):
    """L identifiant TMDB inscrit dans le .mkv : écrit, relu, et pas paye pour rien."""
    ...
    def test_relecture_d_un_identifiant_nu(self):
        # Tolère la forme sans préfixe, qu écrivent d autres outils.
    ...
    def lire(self, info):
        """Lance inspect en notant si mkvextract a été appele."""
        appels = []
        with mock.patch.object(mkv, "identify", lambda p: (info, "")):
            with mock.patch.object(mkv, "read_tags", lambda p: appels.append(p) or {(50, "TMDB", "movie/9")}):
    ...
    def test_aucune_relecture_si_le_fichier_n_a_pas_de_tags(self):
        # Le coût du feature : un mkvextract par fichier. Sur une médiathèque jamais étiquetée, il ne doit jamais être lance.
...
class TestTagsXML(unittest.TestCase):
    ...
    def test_credits_sans_doublon_et_plafonnes(self):
        crew = [{"name": "A", "job": "Director", "department": "Directing"}, {"name": "A", "job": "Director", "department": "Directing"}, {"name": "B", "department": "Writing"}]
...
class TestVerification(unittest.TestCase):
    def setUp(self):
        self.opts = mkv.Options(cover=True, date=True, audio_names=True, sub_names=True, flags=True, stats=True, image_size="w780")
        self.info = {"container": {"properties": {"title": "Pilote", "date_utc": "2024-01-02T00:00:00Z"}}, "tracks": [piste("audio", codec="AAC", audio_channels=2, track_name="AAC 2.0")], "attachments": [{"file_name": "cover.jpg"}]}
    ...
    def test_options_depuis_les_drapeaux(self):
        ...
        self.assertEqual((opts.cover, opts.sub_names, opts.stats, opts.image_size), (False, False, False, "w300"))
...
class TestOutilsExternes(unittest.TestCase):
    ...
    def test_sortie_decodee_en_utf8(self):
        # Régression : sans encodage explicite, Python decode en cp1252 et un caractère absent de cette table (le trait d'union de "est-ce", U+2010) fait échouer la lecture dans un thread de subprocess.
    ...
    def test_sortie_vide_ne_plante_pas(self):
        # C'est ainsi que se manifeste un échec de décodage : stdout à None, code de retour 0, aucune exception.
    ...
    def test_outil_absent_pendant_l_ecriture(self):
        # Régression : la disparition de mkvpropedit remontait en pile d'appels jusqu'à l'utilisateur, au lieu d'être comptée comme un échec d'écriture.
        def absent(cmd, **kwargs):
            raise FileNotFoundError(2, "introuvable", "mkvpropedit")

        with mock.patch.object(mkv.subprocess, "run", absent):
            code, msg = mkv.write("film.mkv", {"tracks": []}, mkv.Target(title="X", tags_xml="<Tags/>"), mkv.Options(cover=False, stats=False, flags=False), None)
...
class TestLectureParallele(unittest.TestCase):
    ...
    def test_remarques_rendues_et_non_imprimees(self):
        # C'est ce qui rend la lecture parallélisable sans entrelacer l'affichage.
...
class TestLectureDesTags(unittest.TestCase):
    def test_cible_absente_vaut_cinquante(self):
        # mkvpropedit omet TargetTypeValue quand il vaut 50, la valeur par défaut : sans cette équivalence, un fichier tout juste écrit paraîtrait différent.
    ...
    def test_aller_retour_avec_ce_qu_on_ecrit(self):
        xml = mkv.tags_document([mkv.tag_block(50, [mkv.simple("TITLE", "Tom & Jerry")]), mkv.tag_block(70, [mkv.simple("PART_NUMBER", 2)])])
        self.assertEqual(mkv.parse_tags(xml), {(50, "TITLE", "Tom & Jerry"), (70, "PART_NUMBER", "2")})
...
class TestVerificationDesTags(unittest.TestCase):
    def setUp(self):
        ...
        self.target = mkv.Target(title="Iron Man", tags_xml=mkv.tags_document([mkv.tag_block(50, [mkv.simple("TITLE", "Iron Man"), mkv.simple("GENRE", "Action")])]))
        self.info = {"container": {"properties": {"title": "Iron Man"}}, "tracks": []}

    def resultat(self, tags):
        return dict((lbl, (ok, det)) for lbl, ok, det in mkv.verify(self.info, self.target, self.opts, tags))
    ...
    def test_tags_absents_signales(self):
        # Régression : un fichier vide de tags était declare conforme, et --skip-done le sautait.
