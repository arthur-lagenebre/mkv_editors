"""Association d'un nom de dossier à une fiche TMDB (films et séries)."""
...
class TestChoixDuResultat(unittest.TestCase):
    def test_remake_signale(self):
        resultats = [{"id": 1, "title": "Dune", "release_date": "2021-09-15"}, {"id": 2, "title": "Dune", "release_date": "1984-12-14"}]
    ...
    def test_titre_exact_prefere_a_la_popularite(self):
        # TMDB classe par popularité : "Blade" rend "Blade II" en premier.
        resultats = [{"id": 36586, "title": "Blade II", "vote_count": 5349}, {"id": 36647, "title": "Blade", "vote_count": 6892}]
    ...
    def test_titre_exact_mais_inconnu_ne_prend_pas_la_place(self):
        # Régression : "The Fast and the Furious" (1954, 41 votes) chassait celui de 2001 (11 029 votes). Un titre exact porte par un inconnu ne vaut rien.
        resultats = [{"id": 9799, "title": "Fast and Furious", "vote_count": 11029}, {"id": 20174, "title": "The Fast and the Furious", "vote_count": 41}]
    ...
    def test_un_classique_moins_vote_que_sa_suite_passe_quand_meme(self):
        # Mad Max (1979, 5 066 votes) face à Fury Road (24 444) : un cinquième, mais c est bien le film demandé. Le seuil doit le laisser passer.
        resultats = [{"id": 76341, "title": "Mad Max : Fury Road", "vote_count": 24444}, {"id": 9659, "title": "Mad Max", "vote_count": 5066}]
    ...
    def test_un_homonyme_confidentiel_ne_passe_pas(self):
        ...
        resultats = [{"id": 122, "title": "Le Seigneur des anneaux : Le Retour du roi", "vote_count": 27139}, {"id": 1361, "title": "Le retour du roi", "vote_count": 223}]
    ...
    def test_le_jumeau_est_signale_meme_apres_le_choix_exact(self):
        resultats = [{"id": 414906, "title": "The Batman", "release_date": "2022-03-01", "vote_count": 10238}, {"id": 268, "title": "Batman", "release_date": "1989-06-23", "vote_count": 8107}, {"id": 2661, "title": "Batman", "release_date": "1966-07-30", "vote_count": 512}]
    ...
    def test_resultat_evident_sans_remarque(self):
        resultats = [{"id": 1, "title": "Inception", "release_date": "2010-07-14"}, {"id": 2, "title": "Inception: The Cobol Job", "release_date": "2010-12-07"}]
        best, notes = lookup.pick_result(resultats, "Inception")
        self.assertEqual((best["id"], notes), (1, []))

    def test_champs_des_series(self):
        resultats = [{"id": 5, "name": "Fargo", "first_air_date": "2014-04-15"}, {"id": 6, "name": "Fargo", "first_air_date": "1997-01-01"}]
    ...
    def test_description(self):
        self.assertEqual(lookup.describe({"id": 27205, "title": "Inception", "release_date": "2010-07-14"}), "Inception (2010) [id 27205]")

    def test_description_sans_date(self):
        self.assertEqual(lookup.describe({"id": 1, "title": "X"}), "X (?) [id 1]")

class FauxFilms:
    """Client TMDB cote films : rend {requête: résultats}, retient les appels."""
    ...
    def search_movie(self, title, year=None):
        # Une clé en texte répond à la recherche sans année ; une recherche avec année ne trouve que ce qu'on lui à explicitement prepare.
...
class TestRenfortDuDossier(unittest.TestCase):
    ...
    def test_le_renfort_ne_passe_jamais_devant_le_titre(self):
        # "DCEU Man of Steel" ne rend rien : le résultat nu reste en place.
    ...
    def test_le_dossier_vient_en_renfort(self):
        # "Folie à deux" ne dit pas de quoi il est la suite ; le dossier, si.
        tmdb = FauxFilms({"Joker Folie a deux": [{"id": 889737, "title": "Joker : Folie a deux"}]})
    ...
    def test_resultat_etranger_cede_au_renfort(self):
        tmdb = FauxFilms({
            ...
            "Batman Aube de la justice": [{"id": 209112, "title": "Batman v Superman : L'aube de la justice", "vote_count": 17000}]})
        resultats, requete = lookup.search_with_context(tmdb, "Aube de la justice", None, "Batman")
        self.assertEqual((resultats[0]["id"], requete), (209112, "Batman Aube de la justice"))

    def test_dossier_qui_repete_le_titre_ne_sert_a_rien(self):
        # Régression : "X-Men/01 - X-Men.mkv" cherchait "X-Men X-Men".
    ...
    def test_une_annee_confirmee_clot_la_question(self):
        # Régression : "_DC/Catwoman (2004)" cherchait "_DC Catwoman" et rapportait le court métrage "DC Showcase: Catwoman". L année écrite dans le nom, et confirmée par la fiche, tranche déjà.
        tmdb = FauxFilms({("Catwoman", "2004"): [{"id": 314, "title": "Catwoman", "release_date": "2004-07-22", "vote_count": 3672}]})
    ...
    def test_une_annee_non_confirmee_laisse_le_renfort_jouer(self):
        tmdb = FauxFilms({("Apocalypse", "2004"): [{"id": 1, "title": "Amour Apocalypse", "release_date": "2025-01-01", "vote_count": 11}], "Resident Evil Apocalypse": [{"id": 1577, "title": "Resident Evil : Apocalypse", "release_date": "2004-09-10", "vote_count": 4880}]})
        resultats, _ = lookup.search_with_context(tmdb, "Apocalypse", "2004", ["Resident Evil"])
        self.assertEqual(resultats[0]["id"], 1577)
    def test_le_dossier_sauve_un_sous_titre_seul(self):
        # Régression : "Apocalypse" ramenait "Amour Apocalypse", un autre film, avec une ressemblance assez bonne pour ne rien déclencher.
        tmdb = FauxFilms({"Apocalypse": [{"id": 1, "title": "Amour Apocalypse", "vote_count": 11}], "Resident Evil Apocalypse": [{"id": 1576, "title": "Resident Evil : Apocalypse", "vote_count": 4880}]})
        resultats, requete = lookup.search_with_context(tmdb, "Apocalypse", None, ["Resident Evil"])
        self.assertEqual((resultats[0]["id"], requete), (1576, "Resident Evil Apocalypse"))

    def test_le_renfort_doit_contenir_le_dossier_ET_le_titre(self):
        # Sinon c est un autre film de la même saga : on ne bouge pas.
        tmdb = FauxFilms({"Apocalypse": [{"id": 1, "title": "Amour Apocalypse"}], "Resident Evil Apocalypse": [ {"id": 2, "title": "Resident Evil : Retribution"}]})
        resultats, requete = lookup.search_with_context(tmdb, "Apocalypse", None, ["Resident Evil"])
        self.assertEqual((resultats[0]["id"], requete), (1, "Apocalypse"))

    def test_le_grand_parent_n_est_pas_essaye(self):
        # Compromis assume : "Resident Evil/Animation/3 - Vendetta" reste mal associe, mais aucun dossier de rangement ne pollue plus les recherches.
        tmdb = FauxFilms({"Vendetta": [{"id": 752, "title": "V pour Vendetta", "vote_count": 12000}], "Resident Evil Vendetta": [{"id": 424781, "title": "Resident Evil : Vendetta", "vote_count": 3200}]})
        resultats, requete = lookup.search_with_context(tmdb, "Vendetta", None, ["Animation", "Resident Evil"])
        self.assertEqual((resultats[0]["id"], requete), (752, "Vendetta"))

    def test_pas_de_renfort_si_le_film_trouve_porte_deja_la_saga(self):
        # "Folie à deux" ramène déjà "Joker : Folie à deux" : rien à ajouter.
        tmdb = FauxFilms({"Folie a deux": [{"id": 889737, "title": "Joker : Folie a deux"}]})
        _, requete = lookup.search_with_context(tmdb, "Folie a deux", None, ["Joker"])
        self.assertEqual((requete, len(tmdb.appels)), ("Folie a deux", 1))

    def test_un_seul_dossier_essaye(self):
        # Au-dessus du dossier immédiat vivent les dossiers de rangement d une médiathèque ("_Marvel"), qui ne sont pas des sagas.
    ...
    def test_un_petit_film_de_saga_passe_quand_meme(self):
        # "Death Race : Anarchy" (399 votes) face à "American Nightmare 2" (6 787) : peu vote, mais c est bien le film du dossier.
        tmdb = FauxFilms({"Anarchy": [{"id": 238636, "title": "American Nightmare 2 : Anarchy", "vote_count": 6787}], "Death Race Anarchy": [{"id": 401478, "title": "Death Race : Anarchy", "vote_count": 399}]})
    ...
    def test_un_renfort_confidentiel_est_refuse(self):
        # Régression : "Les chroniques de Riddick/1 - Pitch Black" troquait Pitch Black (4 914 votes) contre un court métrage de la saga (99 votes).
        tmdb = FauxFilms({"Pitch Black": [{"id": 2787, "title": "Pitch Black", "vote_count": 4914}], "Les chroniques de Riddick Pitch Black": [ {"id": 244839, "title": "Les Chroniques de Riddick : Into Pitch Black", "vote_count": 99}]})
        resultats, requete = lookup.search_with_context(tmdb, "Pitch Black", None, ["Les chroniques de Riddick"])
    ...
    def test_un_dossier_qui_englobe_le_titre_ne_sert_a_rien(self):
        ...
        tmdb = FauxFilms({"Riddick": [{"id": 87421, "title": "Riddick", "vote_count": 4492}]})
        _, requete = lookup.search_with_context(tmdb, "Riddick", None, ["Les chroniques de Riddick"])
    ...
    def test_sans_resultat_nu_le_renfort_est_pris_tel_quel(self):
        ...
        tmdb = FauxFilms({"Les chroniques de Riddick Dark Fury": [{"id": 14663, "title": "Les Chroniques de Riddick : Dark Fury", "vote_count": 300}]})
        resultats, requete = lookup.search_with_context(tmdb, "Dark Fury", None, ["Les chroniques de Riddick"])
    ...
    def test_sans_dossier_pas_de_renfort(self):
        ...
        self.assertEqual((resultats, requete, tmdb.appels), ([], "Inconnu", [("Inconnu", None)]))
...
class TestVariantesDeTitre(unittest.TestCase):
    """Ce qui mérite une question : le même titre écrit autrement."""
    ...
    def test_variante_d_ecriture(self):
        # Le cas qui a motivé la question : trois fiches pour un même titre.
        self.assertEqual(self.rivaux("Les Quatre Fantastiques", "Les Quatre Fantastiques", "Les 4 Fantastiques", "Les 4 Fantastiques"), ["Les 4 Fantastiques", "Les 4 Fantastiques"])

    def test_une_suite_n_est_pas_une_variante(self):
        # "Iron Man 2" rallonge le titre : c est une suite, pas une hésitation.
    ...
    def test_making_of_pris_pour_son_film(self):
        # Cas réel : TMDB classait le making-of avant le film.
        rivaux = self.rivaux("Les gardiens de la galaxie Vol 3", "Rassemblement : Le making-of de Les Gardiens de la Galaxie Vol. 3", "Les Gardiens de la Galaxie : Volume 3")
        self.assertEqual(rivaux, ["Les Gardiens de la Galaxie : Volume 3"])

    def test_deux_films_du_meme_titre_font_une_question(self):
        # "Dracula" en rend trois : rien dans le nom ne les départage.
        res = [{"id": 1246049, "title": "Dracula", "vote_count": 1433}, {"id": 6114, "title": "Dracula", "vote_count": 5875}]
    ...
    def test_un_homonyme_confidentiel_ne_fait_pas_de_question(self):
        # Sans ce filtre, un cinquième de la médiathèque poserait une question : presque tout titre à un homonyme obscur quelque part sur TMDB.
        res = [{"id": 149, "title": "Akira", "vote_count": 5000}, {"id": 999, "title": "Akira", "vote_count": 3}]
    ...
    def test_un_titre_different_n_est_pas_un_jumeau(self):
        res = [{"id": 1, "title": "Dracula", "vote_count": 900}, {"id": 2, "title": "Dracula Untold", "vote_count": 6000}]
        best, _ = lookup.pick_result(res, "Dracula")
        self.assertEqual(lookup.twin_versions(res, best), [])

    def test_liste_de_choix_met_le_plus_vote_en_tete(self):
        # La réponse par défaut - une simple Entrée - doit être la plus vraisemblable.
        res = [{"id": 1, "title": "A", "vote_count": 10}, {"id": 2, "title": "B", "vote_count": 5}, {"id": 3, "title": "C", "vote_count": 900}]
...
class TestNomDeSerie(unittest.TestCase):
    ...
    def test_dossier_de_saison_remonte_au_parent(self):
        # --dir peut pointer sur une saison : c'est le parent qui nomme la série.
...
class TestIdEpingle(unittest.TestCase):
    def resoudre(self, dossier, saison=None, forced=None):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with redirect_stdout(sortie):
                resultat = lookup.resolve_show_id(tmdb, chemin / saison if saison else chemin, forced)
